...
public class CategoriesController implements CategoryControllerInterface {
  ...
  @Override
  public ResponseEntity<?> addCategory(CategoriesRequest categoriesRequest) {
    log.info("Add category for userId: {}", categoriesRequest.userId());
    try {
      CategoryId categoryId = categoriesService.create(categoriesRequest.name(), categoriesRequest.userId());
