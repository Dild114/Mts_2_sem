...
public class CategoriesController implements CategoryControllerInterface {
  ...
  @Override
  public ResponseEntity<?> addCategory(String name, Long userId) {
    log.info("Add category for userId: {}", userId);
    try {
      CategoryId categoryId = categoriesService.create(name, new UserId(userId));
  ...
  @Override
  public ResponseEntity<HttpStatus> deleteCategory(Long id, UserId userId) {
  ...
  @Override
  public ResponseEntity<?> addCategories(Long userId, String[] categories) {
